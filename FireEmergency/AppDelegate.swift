...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        userDefaults.register(defaults: pass)
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey: Any]) -> Bool {
        ...
        if let ext: String? = url.pathExtension.lowercased() {
           //let tempFilePath = NSURL(fileURLWithPath: NSTemporaryDirectory()).URLByAppendingPathComponent("\(NSUUID().UUIDString).\(ext)")
            let tempFilePath = URL(fileURLWithPath: NSHomeDirectory() + "/Documents/fire.csv")
            do {
                try FileManager().moveItem(at: url, to: tempFilePath)
            } catch {
                print("ファイルのコピー失敗")
            }
        } else {
